...
from ebpub.streets.models import Street, City, Block, Intersection, Place, PlaceType, PlaceSynonym

#adding support for search by placename, kdonova1
...
def weather_for_zip(zip_code):
    ...
    ydescription = dom.getElementsByTagName('description')
    ycondition = dom.getElementsByTagNameNS(WEATHER_NS, 'condition')[0]
    return {
        'current_condition': ycondition.getAttribute('text'),
        'description': ydescription[1].firstChild.nodeValue,
...
def homepage(request):
    ...
    weathr = weather_for_zip(44242)
    current_weather = weathr['current_temp']
    weather_img = foo(weathr['description'],'img src="','"/><br')
    ...
    #added by adamDavis for retrieving most recent newsItems to Display
    numberOfNewsItems = 6
    temp = NewsItem.objects.all().order_by("item_date").reverse()
    result = []
    for newsTemp in temp:
    	tempdate=newsTemp.item_date
    	tempSlug=str(newsTemp.schema)
    	if parse_date(tempdate, '%Y%m%d')<=today():
    		if tempSlug!="Photo of the Day":
    			result.append(newsTemp)

    recentNews = result[0:numberOfNewsItems]

    todayDate = today()
    numberOfPhotos = 7
    photo_slug = Schema.objects.get(slug='photos-day')
    photosOfTheDay = NewsItem.objects.all().filter(schema=photo_slug).order_by('pub_date').reverse()[0:numberOfPhotos]
...
def search(request, schema_slug=''):
    ...
    addItem = False
    temp = NewsItem.objects.all().order_by("schema")

    searchResults = []
    for newsTemp in temp:
    	upperTitle = newsTemp.title.upper()
    	upperDescription = newsTemp.description.upper()
    	if searchTerm in upperTitle:
    		addItem = True
    	if searchTerm in upperDescription:
    		addItem = True
    	if addItem:
    		searchResults.append(newsTemp)
    	addItem = False
    ...
    try:
        ...
        if result['ambiguous']:
            if result['type'] == 'block':
                ...
                return eb_render(request, 'db/did_you_mean.html', {
                    'schema_list': s_list,
                    'current_weather': current_weather,
                    'weather_img' : weather_img,
                    'query': q,
                    'search_results' : searchResults, 'choices': result['result']})
        elif result['type'] == 'place':
            block, distance = geocoder.reverse.reverse_geocode(result['result'].location)
            return HttpResponseRedirect(url_prefix + getattr(block, url_method)() + "?q=" + q)
        elif result['type'] == 'location':
            return HttpResponseRedirect(url_prefix + getattr(result['result'], url_method)() + "?q=" + q)
        elif result['type'] == 'address':
            # Block
            if result['result']['block']:
                return HttpResponseRedirect(url_prefix + getattr(result['result']['block'], url_method)() + "?q=" + q)
            # Intersection
            try:
                ...
                return HttpResponseRedirect(url_prefix + getattr(intersection, url_method)() + "?q=" + q)
    ...
    try:
        ...
        if special_case.redirect_to:
            ...
            return eb_render(request, 'db/search_special_case.html', {
                    'schema_list': s_list,
                    'current_weather': current_weather,
                    'weather_img' : weather_img,
                    'query': q,
    ...
    if re.search(r'^\s*\d{5}(?:-\d{4})?\s*$', q):
        ...
        if z_list:
            return eb_render(request, 'db/search_error_zip_list.html', {
                    'schema_list': s_list,
                    'current_weather': current_weather,
                    'weather_img' : weather_img,
                    'query': q,
    ...
    return eb_render(request, 'db/search_error.html', {
                    'schema_list': s_list,
                    'current_weather': current_weather,
                    'weather_img' : weather_img,
                    'query': q,
                    'search_results' : searchResults,
                    'locationtype_list': lt_list})
...
def addCrime(request):
    ...
    if not errors :
        ...
        schema = Schema.objects.get(slug='crimes')
        ...
        #    status = "added"
            ...
            error = "Multiple entries matched title, event titles are not unique?"
        try:
            item.schema = schema
            item.title = title
            item.description = request.POST.get('description', '')
            item.location = request.POST.get('location', '')
            item.item_date = request.POST.get('date', '')
            item.pub_date = request.POST.get('date', '')
        except Exception as e:
            error = "Woops"

        return eb_render(request, 'addCrime.html', {
            'errors' : errors,
...
def about(request):
    ...
    return eb_render(request, 'about.html', {
        'bodyclass': 'about',
    })

def policies(request):

    return eb_render(request, 'policies.html', {
        'bodyclass': 'policies',
    })

def team(request):

    return eb_render(request, 'team.html', {
        'bodyclass': 'team',
    })
...
def schema_detail(request, slug):
    ...
    context['map_configuration'] = _preconfigured_map({'schema_slug': slug, 'start_date': s.min_date, 'end_date': today()})
...
def place_detail_timeline(request, *args, **kwargs):
    ...
    if show_upcoming:
        sl_list = schema_manager.filter(is_event=True)
        start_date = end_date
        end_date = start_date + datetime.timedelta(days=settings.DEFAULT_DAYS)
        order_by = 'item_date_date'
    else:
        sl_list = schema_manager.filter(is_event=False)
        start_date = end_date - datetime.timedelta(days=settings.DEFAULT_DAYS)
        order_by = '-item_date_date'

    filterchain.add('schema', list(sl_list))
    ...
    sl_list = sl_list.filter(is_special_report=False, allow_charting=True).order_by('plural_name')
    ...
    context['filtered_schema_list'] = sl_list
    context['map_configuration'] = _preconfigured_map(context);
